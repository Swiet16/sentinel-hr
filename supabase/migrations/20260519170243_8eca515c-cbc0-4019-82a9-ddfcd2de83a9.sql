...
-- ROLES
-- =========================
create type public.app_role as enum ('super_admin', 'team_leader', 'employee');
...
as $$
  ...
  order by case role when 'super_admin' then 1 when 'team_leader' then 2 else 3 end
  limit 1
$$;
...
create table public.company_settings (
  ...
  attendance_policy jsonb not null default '{"grace_minutes":10,"half_day_after_minutes":120,"overtime_after_hours":8}'::jsonb,
...
create trigger trg_company_settings_updated before update on public.company_settings for each row execute function public.tg_set_updated_at();
...
-- activity_logs
create policy "Users view own activity" on public.activity_logs for select to authenticated
  using (auth.uid() = user_id or public.has_role(auth.uid(), 'super_admin'));
create policy "Authenticated insert activity" on public.activity_logs for insert to authenticated with check (auth.uid() = user_id);

-- notifications
create policy "Users view own notifications" on public.notifications for select to authenticated using (auth.uid() = user_id);
create policy "Users update own notifications" on public.notifications for update to authenticated using (auth.uid() = user_id);
create policy "Super admin manage notifications" on public.notifications for all to authenticated
  using (public.has_role(auth.uid(), 'super_admin')) with check (public.has_role(auth.uid(), 'super_admin'));

-- company_settings
create policy "Settings viewable by authenticated" on public.company_settings for select to authenticated using (true);
...
create policy "Users view own documents" on storage.objects for select to authenticated
